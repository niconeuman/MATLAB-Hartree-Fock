function VAB = recursive_electron_nuclear(a,b,RPA,RPB,RPC,p,RPC2,Z,L1,L2,order,Boys_Table)
...
if (L1 == 0 && L2 == 0)
    ...
    Kab = Ex*Ey*Ez;
    VAB = -1*Z*(2*pi/p)*Interpolated_Boys(order,p*RPC2,Boys_Table)*Kab;
elseif (L1 > 0 && L2 == 0)
    %This line of integrals will always be a COLUMN vector
    VAB_L1_minus_1_order = recursive_electron_nuclear(a,b,RPA,RPB,RPC,p,RPC2,Z,L1-1,L2,order,Boys_Table);
    VAB_L1_minus_1_order_plus_1 = recursive_electron_nuclear(a,b,RPA,RPB,RPC,p,RPC2,Z,L1-1,L2,order+1,Boys_Table);
    if ((L1 - 2) >= 0)
    VAB_L1_minus_2_order = recursive_electron_nuclear(a,b,RPA,RPB,RPC,p,RPC2,Z,L1-2,L2,order,Boys_Table);
    VAB_L1_minus_2_order_plus_1 = recursive_electron_nuclear(a,b,RPA,RPB,RPC,p,RPC2,Z,L1-2,L2,order+1,Boys_Table);
    else
    VAB_L1_minus_2_order = 0;
    VAB_L1_minus_2_order_plus_1 = 0;
    end
    %Check this formula carefully because Eq. 9.10.8 in Helgaker's book
    
    % I NEED to transpose the column vectors into rows
    VAB_matrix = RPA'*VAB_L1_minus_1_order'+1/2/p*(L1-1)*VAB_L1_minus_2_order'...
        -RPC'*VAB_L1_minus_1_order_plus_1-1/2/p*(L1-1)*VAB_L1_minus_2_order_plus_1';
    ...
    elseif (size(VAB_matrix,2) == 3)
        ...
        % yx yy yz;
        % zx zy zz];
            VAB = [VAB_matrix(1,1:3)';VAB_matrix(2,2:3)';VAB_matrix(3,3)];
            %VAB = [xx
            %       xy
            %       xz
            %       yy
    ...
    elseif (size(VAB_matrix,2) == 6) %For now (6/01/2017) I won't use more than f functions, so I don't think I need more for now
        ...
        VAB = [VAB_matrix(1,1:6)';VAB_matrix(2,4:6)';VAB_matrix(3,6)];
...
elseif (L1 == 0 && L2 > 0)
    ...
    VAB_L2_minus_1_order = recursive_electron_nuclear(a,b,RPA,RPB,RPC,p,RPC2,Z,L1,L2-1,order,Boys_Table);
    VAB_L2_minus_1_order_plus_1 = recursive_electron_nuclear(a,b,RPA,RPB,RPC,p,RPC2,Z,L1,L2-1,order+1,Boys_Table);
    if ((L2 - 2) >= 0)
    VAB_L2_minus_2_order = recursive_electron_nuclear(a,b,RPA,RPB,RPC,p,RPC2,Z,L1,L2-2,order,Boys_Table);
    VAB_L2_minus_2_order_plus_1 = recursive_electron_nuclear(a,b,RPA,RPB,RPC,p,RPC2,Z,L1,L2-2,order+1,Boys_Table);
    else
    ...
    elseif (size(VAB_matrix,2) == 3)
        ...
        % yx yy yz;
        % zx zy zz];
            VAB = [VAB_matrix(1,1:3)';VAB_matrix(2,2:3)';VAB_matrix(3,3)]'; %L = 2
            %VAB = [xx
            %       xy
%                   xz
%                   yy
...
%                   zz];
    elseif (size(VAB_matrix,2) == 6) %For now (6/01/2017) I won't use more than f functions, so I don't think I need more for now
        ...
        % Yxx,Yxy,Yxz,Yyy,Yyz,Yzz;
        % Zxx,Zxy,Zxz,Zyy,Zyz,Zzz];
        VAB = [VAB_matrix(1,1:6)';VAB_matrix(2,4:6)';VAB_matrix(3,6)]'; %L = 3
%     elseif (size(VAB_matrix,2) == 10)
%         %[Xxxx,Xxxy,Xxxz,Xxyy,Xxyz,Xxzz,Xyyy,Xyyz,Xyzz,Xzzz;
%         % Yxxx,Yxxy,Yxxz,Yxyy,Yxyz,Yxzz,Yyyy,Yyyz,Yyzz,Yzzz;
%         % Zxxx,Zxxy,Zxxz,Zxyy,Zxyz,Zxzz,Zyyy,Zyyz,Zyzz,Zzzz];
