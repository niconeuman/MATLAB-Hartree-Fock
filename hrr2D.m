...
elseif L2 == 1 %[pp|ss], [dp|ss], [fp|ss], etc
    ...
    elseif (L1 == 0 && L3 > 1 && L4 == 0)
        if (L3 > L2)
           gabcd = contract_ascs(basis_c,basis_d,basis_b,basis_a,L3,L4,L2,L1,Boys_Table,nz,unique);
           gabcd = permute(gabcd, [1 3 2 4]);
           gabcd = permute(gabcd, [3 4 1 2]); %remember ascs produces a 2D matrix. So permutation of L3 and L1 transposes the matrix
           gabcd = permute(gabcd, [2 1 3 4]);
        else
           gabcd = contract_ascs(basis_b,basis_a,basis_c,basis_d,L2,L1,L3,L4,Boys_Table,nz,unique);
           gabcd = permute(gabcd, [1 3 2 4]);
           gabcd = permute(gabcd, [2 1 3 4]);
        end
    elseif (L1 == 0 && L3 == 0 && L4 > 1)
        if (L4 > L2)
           gabcd = contract_ascs(basis_d,basis_c,basis_b,basis_a,L4,L3,L2,L1,Boys_Table,nz,unique);
           gabcd = permute(gabcd, [1 3 2 4]);
           gabcd = permute(gabcd, [3 4 1 2]); %remember ascs produces a 2D matrix. So permutation of L3 and L1 transposes the matrix
           gabcd = permute(gabcd, [2 1 4 3]);
        else
           gabcd = contract_ascs(basis_b,basis_a,basis_d,basis_c,L2,L1,L3,L4,Boys_Table,nz,unique);
           gabcd = permute(gabcd, [1 3 2 4]);
           gabcd = permute(gabcd, [2 1 4 3]);
        end        
    else %(L1 == 0 && L3 > 0 && L4 > 0)
